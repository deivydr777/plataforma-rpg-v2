...
// Recebe `communities` e `addCommunity` como props do App.js
function CommunitiesScreen({ communities, addCommunity }) {
  ...
  const handleCreateCommunity = () => {
    ...
    if (trimmedName) {
      ...
      addCommunity(newCommunity); // Usa a função do App.js para adicionar a comunidade
  ...
  // Navega para a visão detalhada da comunidade
      ...
      {communities.length > 0 ? (
        <CommunityList>
          {communities.map(community => (
            <CommunityItem key={community.id} onClick={() => handleCommunityClick(community)}>
...
const Header = styled.header`
  ...
  position: sticky; 
  top: -20px; /* Truque para o padding não criar espaço extra no topo */
  z-index: 10; 
